...
materialPath = path + 'RP/textures/blocks/storage/uranium_'#Path to selected material (Used material_ at the end to complement the following part)

for i in ['block']:#Write molds to do P.D. the name of the mold has to match an existing image in the 'materialPath' (I complement this by putting the path to the material plus the start of all the files in it which is the material and a '_')
    ...
    for y in range(16):
        ...
        for x in range(16):
            for shade in range(10): #Check if that pixel is in the range of shades
                if len(moldIMG[y][x]) == 3:
                    moldIMG[y][x].append(255) #Blocks are only RGB and not RGBA
